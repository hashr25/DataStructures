...
{
	public class BlockMapGeneration : MonoBehaviour
	{
		...
		void InitialiseList ()
		{
			...
			for(int x = 1; x < columns-1; x++)
			{
				...
				for(int y = 1; y < rows-1; y++)
				{
					...
					gridPositions.Add (new Vector3(x*5, 0f, y*5));
				}
			}
		}
		...
		void BoardSetup ()
		{
			...
			for(int x = -1; x < columns + 1; x++)
			{
				...
				for(int y = -1; y < rows + 1; y++)
				{
					...
					GameObject instance =
						Instantiate (toInstantiate, new Vector3 (x*5, 0f, y*5), Quaternion.identity) as GameObject;
